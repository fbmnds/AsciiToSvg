module AsciiToSvgScaffolding.Tests
...
let ``FsCheck test 2 (string generator)``() =
    let genString = gen { let! a = Arb.generate<string>
                          return a }
    ...
    Check.One("Arbitrary strings", nUnitConfig,
              (Prop.forAll (Arb.fromGen genString) (fun myString ->
                   myString = myString + " "
...
[<Test>]
let ``FsCheck test 6a (stateful testing spec)``() = Check.QuickThrowOnFailure(asProperty spec)
...
[<Test>]
let ``FsUnit test 1``() = [ 1; 2; 3 ] |> should equal [ 1; 2; 3; 4 ]

[<Test>]
let ``FsUnit test 2``() = 1 |> should not' (equal 1)

[<Test>]
let ``FsUnit test 3``() = 10.1 |> should (equalWithin 0.1) 10.22

[<Test>]
let ``FsUnit test 4 (should throw exception)``() = (fun () -> 1 + 2 |> ignore) |> should throw typeof<System.Exception>

[<Test>]
let ``Unquote test 1``() = test <@ ([ 3; 2; 1; 0 ] |> List.map ((+) 1)) = [ 1 + 3..1 + 0 ] @>
